...
from tkinter import *
sleepTime, INFINITY, AU_OFFSET_CONSTANT, HI_OFFSET_CONSTANT, W_OFFSET_CONSTANT, L_OFFSET_CONSTANT, LF_OFFSET_CONSTANT, mnemofile, ignorefile, tokenfile, raspyafile, cachefile, headerfile, delayfile, looping, photosizes, printm, width, height, mnemonics, ignore, header, idscache, uidscache, lastNviewcache, prob, token_num, block, delayed, default, full_auth_line = 0.34, 10000000, 300, 200, 100, 1000, 100, 'mnemo.txt', 'ignore.txt', 'tokens.txt', 'rasp.ya.txt', 'cache.txt', 'header.txt', 'delay.txt', False, [2560, 1280, 807, 604, 512, 352, 256, 130, 128, 100, 75, 64], '', 0, 0, {}, [], '', [], {}, [], [], 0, [], [], (0, 0, 53, 500, 1, False), 'https://oauth.vk.com/authorize?client_id=5193865&scope=notify,friends,photos,audio,video,docs,notes,pages,status,offers,questions,wall,groups,messages,notifications,stats,ads,offline&redirect_uri=https://oauth.vk.com/blank.html&display=page&response_type=token'
...
def saveinstance():
        with open(cachefile, 'w', encoding='utf-8') as cache_file:
                cache_file.write('\n'.join([str(idscache), str(lastNviewcache), str(prob), str((token_num, prevuserid, waitTime, TOO_LONG_CONSTANT, lang, hide)), str(uidscache)]))
def reset():
        global token_num, idscache, uidscache, lastNviewcache, prob, prevuserid, token_list, sleepTime, waitTime, TOO_LONG_CONSTANT, lang, hide
        lastNviewcache, idscache, uidscache, prob, token_num, prevuserid, waitTime, TOO_LONG_CONSTANT, lang, hide = [], [], {}, [], *default
        if not token_list:
        ...
        return 0
def getcache():
        global idscache, lastNviewcache, prob, token_num, prevuserid, waitTime, TOO_LONG_CONSTANT, lang, hide, uidscache
        with open(cachefile, 'r', encoding='utf-8') as cache_file:
                try:
                        i, l, p, cort, u = cache_file.readlines()
                        idscache, lastNviewcache, prob, token_num, prevuserid, waitTime, TOO_LONG_CONSTANT, lang, hide, uidscache = ast.literal_eval(i), ast.literal_eval(l), ast.literal_eval(p), *ast.literal_eval(cort), *default[cort.count(',')+1:], ast.literal_eval(u)
                except: reset()
def getcached(uid):
        global uidscache
...
def add_delayed(delay_time):
        ...
        block = [] 
def messaging():
        global token_num, printm, waitTime, prevuserid, block, lang, TOO_LONG_CONSTANT, hide, header
        iam()
                ...
                if (m==''):
                        ...
                        if (len(s)==1):
                                ...
                                if r(">") or r("<"):
                                        ...
                                        if s is None: return(0)
                                        if r("?"):
                                                print(help("<") if repeated else nowstamp.strftime(help(">")+' 18:00:00 %d.%m.%Y'))
                                                return(0)
                                        if repeated:
                                ...
                                if r("{"):
                                        ...
                                        if s[0].isdigit(): forward_messages += ','+s
                                                ...
                                                print(help("subj1")+s[2:]+help("subj2"))
                                        else: attachments += ','+s
                                        continue
                                if r("~") or r("`"):
                                        ...
                                        if s is None: return(0)
                                        if r("?"):
                                                print('[wget - to download using wget]\nopens a file from a direct link. Or opens tokenfile, mnemofile, ignorefile, raspyafile - using T,M,I,R commands')
                                                return(0)
                                        owget = r("wget")
                                        if owget:
                                                s = cin()
                                        ...
                                        if s.startswith('doc'):
                                                ...
                                                print("photo123123_123123 or audio1231231_12213 or doc12412_34234 or video2123_123123 or http://vk.com/video_ext.php?oid=...")
                                                return(0)
                                        ...
                                        r.encoding = 'UTF-8'
                                        x = r.text
                                        p = x.find('Поздравляем')
                                        inf = x[p:x.find('Регион в Яндекс.Паспорте', p)]
                                        print(''.join(re.split('<[\/\!]*?[^<>]*?>', inf))[:-1])
                                        return(0)
                                elif r("g"):
                                        s = cin()
                                        if s is None: return(0)
                                        gooreq = requests.get("https://www.google.ru/search?q="+s)
