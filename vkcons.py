...
def getcache():
        ...
        with open(cachefile, 'r', encoding='utf-8') as cache_file:
                try:
                        i, v, p, cort, u = cache_file.readlines()
                        idscache, lastNviewcache, prob, token_num, prevuserid, waitTime, TOO_LONG_CONSTANT, lang, hide, confirmation_word, uidscache = ast.literal_eval(i), ast.literal_eval(v), ast.literal_eval(p), *ast.literal_eval(cort), *default[cort.count(',')+1:], ast.literal_eval(u)
                except:
                        if token_list:
                        ...
                        if (len(s)==1):
                                ...
                                if r("~"):
                                        ...
                                        for post in wall:
                                                ...
                                                if not sugpone: ls, cs = post.get('likes').get('count'), post.get('comments').get('count')
                                                printsn('\n'+posttype.upper()+'\n____' if sugpone else '\n'+str(ls)+' '+help("like")+help("decl_3a")[flex(ls)]+', '+str(cs)+' '+help("comment")+help("decl_7a")[flex(cs)]+'\n____')
                                        if len(wall)==1:
                                                printms()
                                                com_num = post.get('comments').get('count')
                                                if com_num>10:                                        
                                                        print(help("comments")[0] if hide else help("comments")[1] + str(com_num) + help("comments")[2] + help("decl_7a")[flex(com_num)] + help("comments")[3])
                                                        s = cin()
                                                        if s is None: return(0)
                                                else: s="y"
                                                if not sugpone and (r("y") or r("s")):
                                                        printsn(help('COMMENTS'))
...
def check_inbox():
        ...
        for token_num in range(len(token_list)):
                ...
                a=r+t
                A+=a
                printsn(myname + ' - ' + str(t) + ' '+help("chx")[0] + help('decl_3a')[flex(t)] + ' - ' + str(r) + ' ' + help("chx")[1] + help('decl_3a')[flex(r)])
                if r<0: A-=r
                if not looping:
                        if a: printms()
                        else: printm='' #print()
                for x in items:
                        ...
                        if respname: printsn('\n'+respname.get('first_name')+' '+respname.get('last_name')+' '+str(uid)+' '+str(N)+' '+help("message")+help("decl_7a2")[flex(N)])
